...
**_ TWO-LAYER PIPELINE — see full spec below _**
...
Package everything spaCy produced: - Expanded (abbreviation-free) text - Section labels per sentence - negated_spans[] list
...
ABDM-SPECIFIC FHIR PROFILES: - OPD Consultation Note - Prescription Bundle - Diagnostic Report Bundle - Immunization Record - Health Document Record (discharge summaries)
...
STEP 17 — Test full backend: upload discharge_complex.pdf, verify: - "No h/o TB" excluded from all outputs - spaCy negation_spans passed correctly to Gemini - Post-processing override working - Drug brand names mapped to INN
...
def preprocess(raw*text: str) -> dict:
"""Layer 1 — spaCy"""
...
negated_spans = [
e.text for e in doc.ents if e.*.is_negated # ConText
]
return {
...
════════════════════════════════════════════════════════════
REAL-WORLD FLOW MAPPING (FROM HOSPITAL PROCESS)
════════════════════════════════════════════════════════════
Your uploaded handwritten flow = actual TPA hospital lifecycle.
We map NormClaim into that flow:

🧩 EXISTING HOSPITAL FLOW (SIMPLIFIED)
Pre-auth request
Pre-auth approval
Admission
Enhancement request
...
🚀 WHERE NORMCLAIM PLUGS IN
NormClaim is NOT replacing the system — it is augmenting 3 critical leakage points:
🔴 POINT 1 — DISCHARGE SUMMARY CREATION (BIGGEST LEAK)
Current:
Doctor writes free-text discharge summary
↓
Billing clerk manually codes ICD-10
↓
Misses comorbidities
NormClaim replaces this with:

Discharge PDF → NLP Pipeline → Structured Diagnoses → ICD-10 complete set
✅ Output:
All primary + secondary diagnoses
No missed comorbidities
Negations removed (critical)
🟠 POINT 2 — CLAIM STRUCTURING (TPA SUBMISSION)
Current:
Manual mapping to packages
Incorrect DRG grouping
Under-claiming
NormClaim adds:
ICD-10 set → DRG logic → Optimized claim structure
✅ Output:
Maximum valid claim value
Package alignment
Audit-safe structured claim
🔵 POINT 3 — FINAL SETTLEMENT RECONCILIATION
Current:
Finance checks manually
No idea what was missed
NormClaim adds:

Original bill vs Extracted codes → Delta engine
✅ Output:
₹ delta report
...
🧠 FINAL POSITIONING
NormClaim is:
"A post-discharge intelligence layer that maximizes claim value and ensures ABDM compliance automatically."
════════════════════════════════════════════════════════════
END-TO-END PRODUCT FLOW (FINAL)
════════════════════════════════════════════════════════════
[UPLOAD]
↓
PDF / Image Upload (Hospital Staff / Admin)

↓
[STAGE 1–2]
Digitization Layer
(pdfplumber / OCR / Gemini Vision)
...
↓
[STAGE 6]
FHIR Bundle Generation (ABDM compliant)

↓
[STAGE 7]
Claim Structuring Engine
...
- Compare vs original bill
- Detect missed diagnoses
- Calculate ₹ delta
...
1. FHIR Bundle (ABDM ready)
2. Optimized Claim
3. ₹ Delta Report
4. Audit Trail
   ════════════════════════════════════════════════════════════
   CORE INNOVATION (WHAT JUDGES CARE ABOUT)
   ════════════════════════════════════════════════════════════
5. 🔒 Deterministic Negation (spaCy ConText)
   Prevents false billing (legal risk)
   This is your strongest technical point
6. 🇮🇳 India-Specific NLP Handling
   Hinglish → English medical mapping
   Indian abbreviations
   Brand → generic drugs
7. 💰 Revenue Recovery Engine
   Not just extraction → financial impact
   Direct ₹ value output
8. 🏥 ABDM Compliance Layer
   FHIR R4 bundle auto-generation
   Future-proof for 2026 mandate
9. 🔁 Learning System (Feedback Loop)
   Human corrections → stored → improves prompts
   ════════════════════════════════════════════════════════════
   MVP SCOPE (HACKATHON REALISTIC CUT)
   ════════════════════════════════════════════════════════════
   MUST BUILD (DEMO CRITICAL)
   PDF upload
   spaCy + Gemini pipeline
   ICD-10 extraction
   Negation handling
   ₹ delta calculation
   Basic FHIR bundle
   Simple review UI
   CAN FAKE / SIMPLIFY
   DRG mapping (use static mapping table)
   ABDM full validation (basic structure enough)
   Multi-format ingestion (focus PDF only)
   ════════════════════════════════════════════════════════════
...
🚫 TB correctly excluded

ONE LINE IMPACT:
“We don’t just digitize claims — we increase hospital revenue by 20–40% per discharge.”
════════════════════════════════════════════════════════════
FINAL TAGLINE
════════════════════════════════════════════════════════════
NormClaim = Extract → Normalize → Optimize → Comply
If you say "next", I’ll start implementing:
👉 backend/services/nlp_preprocessor.py (spaCy layer, production-ready)
